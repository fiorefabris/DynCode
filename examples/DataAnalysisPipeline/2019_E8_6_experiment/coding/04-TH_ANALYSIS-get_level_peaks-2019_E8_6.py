...
def main(dat,nprocs,df_dic,filename_dat,key,target_conc): 
    ...
    for (v,res) in zip(values,results):
        if v[0] == key[3]:
            df_results_20ng[tuple([str(x) for x in v[1]])] = res        
        if v[0] == key[2]:
            df_results_5ng[tuple([str(x) for x in v[1]])] = res
        if v[0] == key[1]:
            df_results_2_5ng[tuple([str(x) for x in v[1]])] = res
        if v[0] == key[0]:
            df_results_CN[tuple([str(x) for x in v[1]])] = res
            
    filename_20ng = 'df_results_curve_'+target_conc[3]+'_'+filename_dat[-11:]
    filename_5ng  = 'df_results_curve_'+target_conc[2]+'_'+filename_dat[-11:]
    filename_2_5ng = 'df_results_curve_'+target_conc[1]+'_'+filename_dat[-11:]
    filename_CN = 'df_results_curve_NC_'+filename_dat[-11:]

    save_data(filename_20ng,df_results_20ng)
    save_data(filename_5ng,df_results_5ng)
    save_data(filename_2_5ng,df_results_2_5ng)
    save_data(filename_CN,df_results_CN)
...
for level in ['level_1.pkl']:
    filename_dat = 'NC_'+level

    infile = open(filename,'rb')
    df_dic = pickle.load(infile)
    infile.close()
    
    infile = open(filename_dat,'rb')
    dat = pickle.load(infile)
    infile.close()
    
    
    target_conc = ['NC','2-5ng','5ng','20ng']
