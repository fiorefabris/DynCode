...
def main(amp_th,pendiente_th,nprocs,df_dic): 
    ...
    df_results_0ng = pd.DataFrame(index=amp_th,columns = pendiente_th)
    df_results_2_5ng = pd.DataFrame(index=amp_th,columns = pendiente_th)
    df_results_5ng = pd.DataFrame(index=amp_th,columns = pendiente_th)
    df_results_20ng = pd.DataFrame(index=amp_th,columns = pendiente_th)


    for (v,res) in zip(values,results):
        if v[0] == 'an_0ng':
            df_results_0ng.loc[v[1],v[2]] = res
        if v[0] == 'an_2-5ng':
            df_results_2_5ng.loc[v[1],v[2]] = res
        if v[0] == 'an_5ng':
            df_results_5ng.loc[v[1],v[2]] = res
        if v[0] == 'an_20ng':
            df_results_20ng.loc[v[1],v[2]] = res

            
    filename_0ng = 'df_results_0ng_Norm_.pkl'
    filename_2_5ng = 'df_results_2_5ng_Norm_.pkl'
    filename_5ng = 'df_results_5ng_Norm_.pkl'
    filename_20ng = 'df_results_20ng_Norm_.pkl'
    

    outfile_0ng= open(filename_0ng,'wb')
    pickle.dump(df_results_0ng,outfile_0ng)
    outfile_0ng.close()
    
    outfile_2_5ng= open(filename_2_5ng,'wb')
    pickle.dump(df_results_2_5ng,outfile_2_5ng)
    outfile_2_5ng.close()

    outfile_5ng= open(filename_5ng,'wb')
    pickle.dump(df_results_5ng,outfile_5ng)
    outfile_5ng.close()
    
    outfile_20ng= open(filename_20ng,'wb')
    pickle.dump(df_results_20ng,outfile_20ng)
    outfile_20ng.close()
...
filename= '2019_E8_6.pkl'
#amp_th= np.linspace(0,3000,30)
amp_th= np.linspace(0,2000,2)
#pendiente_th =  np.linspace(0,500,30)
pendiente_th =  np.linspace(0,500,2)
nprocs = 4 #72

infile = open(filename,'rb')
df_dic = pickle.load(infile)
infile.close()
