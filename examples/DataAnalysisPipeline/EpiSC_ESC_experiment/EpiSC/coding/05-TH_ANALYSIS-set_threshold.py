...
data_folder = '/home/fabris/Documents/Trabajo/Doctorado/Dyncode/DO2020/EpiSC_ESC_Experiment/EpiSC/data/data_explore_th/'
...
#OLD: c_levels= [1e-4]
#level 1 : (42, 406) #('42.97191138371258', '406.77966101694915')

#th_explore: c_levels [1e-3,5e-3,1e-2]
# level 1: (33, 305) + ('33.1315210671255', '305.08474576271186')
#level 2:  (16, 298) + ('16.949152542372882', '298.9889518655964')
# level 3: (30, 203) + ('30.669759573667037', '203.38983050847457')

fig = plt.figure(constrained_layout=False, figsize=(8.27, 11.69))
gs_main = gridspec.GridSpec(nrows=2, ncols=1, figure=fig); gs_main.update(left=0.1, right=0.9, bottom=0.2, top=0.90, hspace=0.0)

ax2 = plt.subplot(gs_main[1]);


for k,l in enumerate(['level_3']):
    ...
    th_i = np.argmax([np.mean(i) for i in box])    
    ax2.plot(th_i, 0.035, marker="v", markersize=4, alpha=1, color="red",linewidth = 0)
    ...
    ax2.set_ylim([0,4e-2]); #ax2.set_yticks([0,0.01,0.03,0.05,0.1])
    ...
    ax2.set_yticklabels([0,4e-2/20*60],rotation = 0)
...
plt.savefig(save_folder+l+'_level_curve.pdf', format='pdf')

print('threshold: ' + str(label[th_i]))
...
main_folder = '/home/fabris/Documents/Trabajo/Doctorado/Dyncode/DO2020/EpiSC_ESC_Experiment/EpiSC/data/data_explore_th/'
conc_data = {}
key =[ 'an_EpiSC_FAX_PD03', 'an_EpiSC_FAX']
keywords = ['CN','WT']
 

TH = ('30.669759573667037', '203.38983050847457')
slope_th = 30
amp_th = 203
level = 'level_3'
...
for k,c in enumerate([ 'an_EpiSC_FAX_PD03', 'an_EpiSC_FAX']):
    df = conc_data[c]
    ...
    for cell in cells:
        ...
        if cells[-1] >= cell:
            ...
            ax.plot(df.loc[cell]['max_'].values, 'o', color='b', markersize=0.3, alpha=1)
