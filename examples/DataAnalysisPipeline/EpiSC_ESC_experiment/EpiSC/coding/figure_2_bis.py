...
filename= '/home/fabris/Documents/Trabajo/Doctorado/Dyncode/DO2020/EpiSC_ESC_Experiment/EpiSC/data/EpiSC.pkl'
conc = load_file(filename)

main_folder = '/home/fabris/Documents/Trabajo/Doctorado/Dyncode/DO2020/EpiSC_ESC_Experiment/EpiSC/data/'
conc_data = {}
key =[ 'an_EpiSC_FAX_PD03', 'an_EpiSC_FAX']
keywords = ['CN','WT']
TH = ('42.97191138371258', '406.77966101694915')#(42, 406) #
slope_th = 42
amp_th = 406
...
#%%
save_folder = '/home/fabris/Documents/Trabajo/Doctorado/Dyncode/DO2020/EpiSC_ESC_Experiment/EpiSC/explore_th_figures/'
...
l = ['*','**','***','****','*****','*','**','***','****','*****']
traze_label = [0,1,2]
cells_conc = {'an_EpiSC_FAX':[27,46,50],'an_EpiSC_FAX_PD03':[5,14,25]} 
conc_labels = {'an_EpiSC_FAX_PD03':'EpiSC Meki','an_EpiSC_FAX': 'EpiSC (FAX)'}
...
for k,c in enumerate(ipi_conc):
    ...
    bins1 = ax1.hist(df[col3],bins=np.arange(2,140,12),range=(2,None), density=1, facecolor=colors[k], alpha=0.8,linewidth=0.0 )
    bins3 = ax3.hist(df[col2],bins=np.arange(2,24,2),range=(2,None), density=1, facecolor=colors[k], alpha=0.8,linewidth=0.0 )
    bins2 = ax2.hist(df[col1],bins=20,range =(amp_th,1000), density=1, facecolor=colors[k], alpha=0.8,linewidth=0.0 )
    ...
    ax1.set_xlim([0,140]);ax1.set_ylim([0,0.04])
    ax2.set_xlim([0,1000]);ax2.set_ylim([0,0.005]);
    ax3.set_xlim([0,24]);ax3.set_ylim([0,0.15]);
...
set_scale(ax1,np.arange(0,140,30),[0,0.04]); ax1.set_xlabel('IPI (min)',fontsize=8); ax1.set_ylabel('probability',fontsize=8,rotation = 90)
...
ax1.set_yticklabels([np.round(x,1) for x in [0,0.04*3]],fontsize=6)


set_scale(ax2,[amp_th,1000],[0,0.005]); ax2.set_xlabel('amplitude (a.u.)',fontsize=8); ax2.set_ylabel('probability',fontsize=8,rotation = 90)
ax2.xaxis.set_label_coords(0.5, -0.2);ax2.yaxis.set_label_coords(-0.01,0.5)
ax2.set_xticklabels([amp_th/100,1000/100],fontsize=6)

set_scale(ax3,np.arange(0,24,5),[0,0.15]); ax3.set_xlabel('duration (min)',fontsize=8); ax3.set_ylabel('probability',fontsize=8,rotation = 90)
...
ax3.set_yticklabels([np.round(x,1) for x in [0,0.15*3]],fontsize=6)
...
# 
# =============================================================================
plt.savefig(save_folder+'level_3_Figure2.pdf', format='pdf')
