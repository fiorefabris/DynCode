...
def get_heatmap(df_result,condition,save=False,save_folder=None):
    ...
    hm = sns.heatmap(df_result.fillna(-1),annot=False,annot_kws={'fontsize':5},
                cbar=True,ax = ax,vmin=0.00001,vmax=0.05,cbar_kws={'label': r'Number of peaks x min '+
                                            ' \n (averaged x cells)' ,'extend': 'both','ticks':[0.00001, 0.01,0.05]},cmap="RdYlBu_r",
                norm=SymLogNorm(vmin=0.00001,vmax=0.05,linthresh=0.005,linscale=0),square=True)
    ...
    set_scale(ax,np.linspace(0,1000,6), np.linspace(0,1000,5))
    ax.xaxis.set_major_formatter(ticker.FixedFormatter([str(x) for x in np.arange(0,6000,1000)]))
    ax.yaxis.set_major_formatter(ticker.FixedFormatter([str(x) for x in np.arange(0,500,100)]))
    #ax.set_yticks(np.arange(0,6000,1000))
    #ax.set_yticklabels([x/100 for x in np.arange(0,6000,1000)] ,rotation = 0)
    #ax.set_xticks( np.arange(0,500,100))
   # ax.set_xticklabels([x/100*3 for x in np.arange(0,500,100)],rotation = 0)
    ax.set_ylabel('Amplitude threshold (a.u.)',fontsize=15); 
    ax.set_xlabel('Slope threshold (a.u./min)',fontsize=15);
    ...
    cbar.ax.set_yticklabels([np.round(x*3,5) for x in cbar.get_ticks()])
    
    max_value= round(max(df_result.fillna(-1).max()),4)
    ax.text(1.1, 1.05,'Max value: '+str(np.round(max_value*3,2)) + ' 1/min', ha='center', va='center', transform=ax.transAxes, fontsize=10)
    fig.subplots_adjust(bottom=0.20, top=0.9, left=0.15, right=0.9,wspace=0.0, hspace=0.0)

    if save:
        plt.savefig(save_folder + 'heatmap_'+condition+'.pdf', format='pdf')
    else:
        plt.show()
#%%
...
def get_level_curves(heatmap_values,heatmap_condition,
    ...
    levels=np.arange(1e-5,0.01+5e-5,5e-5)
    CSF = ax.contourf(xx, yy, Z,levels=levels,extend='both',cmap='RdYlBu_r',vmin=1e-5,
                      vmax=0.01,norm=SymLogNorm(vmin=1e-5,vmax=0.01,linthresh=1e-5,linscale=0))

    c_levels= [5e-4]
    #c_levels= [1e-4]

    CS = ax.contour(xx, yy, lcurves_values,levels=c_levels,norm=SymLogNorm(vmin=1e-5,vmax=0.01,linthresh=1e-1), colors='green')
    ...
    axs.set_yticks(np.arange(0,6000,1000))
    axs.set_yticklabels([x/100 for x in np.arange(0,6000,1000)] ,rotation = 0)
    axs.set_xticks( np.arange(0,500,100))
    axs.set_xticklabels([x/100*3 for x in np.arange(0,500,100)],rotation = 0)
    axs.set_ylim([0,3000]);axs.set_xlim([0,300])
    
    cbar_ticks=[1e-5,1e-4,1e-3,5e-3,1e-2,0.05]
    cbar = fig.colorbar(CSF, ax=ax,label= r'Number of peaks x min'+' \n (averaged x cells)', 
                        ticks= cbar_ticks,spacing='proportional')
    cbar.add_lines(CS)
    cbar.ax.tick_params(labelsize=10)
    cbar.ax.yaxis.label.set_size(15)
    cbar.ax.set_yticklabels([np.round(x*3,5) for x in cbar_ticks])
    ...
    if save_lcurves:
        for i,level in enumerate(range(len(CS.allsegs))):
            if CS.allsegs[level] == []:
                ...
                dat0= CS.allsegs[level][0]
                filename = save_folder+'e_'+lcurves_condition+'_level_'+str(level+1)+'.pkl'
